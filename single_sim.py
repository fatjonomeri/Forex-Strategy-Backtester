...
stoch_K = 50
stoch_D = 50
possible_fib_levels = [0, 0.382, 0.5, 0.618,
                       0.786, 1, -0.236, -0.5, -0.618, -1, -1.236]

SL_ENTRY_TP = [0.618, -0.213, -2.5]
...
def run():
    ...
    for pairname in test_pairs:
        ...
        pipLoc = instrument.Instrument.get_pipLocation(pairname)
        ...
        df, score = modify_df(df_trades, m5_data, pipLoc)
        grand_total += score
        print(f"{pairname} {score:.0f}")
        df.to_pickle(f"{pairname}_pip_{granularity}.pkl")
        df.to_pickle("GBP_USD_H4_df.pkl")
